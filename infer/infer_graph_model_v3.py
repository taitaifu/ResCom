...
from models.data_utils_v3 import (  # noqa: E402
    GraphTemporalSequenceDatasetV3,
    GroupStandardizer,
    NumpyStandardScaler,
    WHEEL_IDS,
    check_columns_exist,
...
from train.train_graph_model_v3 import (  # noqa: E402
    apply_axis_gate,
...
def log_status(message: str) -> None:
    print(f"[infer_v3] {message}", flush=True)
...
def inference_required_columns(spec, available_cols: set[str], case_col: str, time_col: str) -> List[str]:
    ...
    terrain_aliases = ["Kc", "Kphi", "n0", "n1", "c", "phi", "K", "cohesion", "shear_K"]
    terrain_aliases.extend([f"terrain_{name}" for name in ["Kc", "Kphi", "n0", "n1", "c", "phi", "K"]])
    terrain_aliases.extend([f"soil_{name}" for name in ["Kc", "Kphi", "n0", "n1", "c", "phi", "K"]])
    terrain_aliases.append("K_c")
    for wheel_id in WHEEL_IDS:
        required.extend([f"lf_wheel{wheel_id}_{name}" for name in terrain_aliases])
    required.extend(terrain_aliases)
...
def restore_scaler_v3(checkpoint_path: str, ckpt: Dict) -> GroupStandardizer:
    ...
    for name in ["group_scaler_v3.joblib", "group_scaler_v2.joblib", "group_scaler.joblib"]:
        path = ckpt_dir / name
        if path.exists():
            return GroupStandardizer.load(str(path))
    raise FileNotFoundError(f"未在 checkpoint 或 {ckpt_dir} 下找到 group_scaler_v3.joblib")


def resolve_model_role_v3(model_role: str, ckpt: Dict) -> str:
    if model_role != "auto":
        return model_role
...
def build_model_from_checkpoint_v3(
    ckpt: Dict,
    spec,
    model_role: str,
    device: torch.device,
) -> Tuple[GraphTemporalHGTCompensationModelV3, Dict]:
...
def build_v3_contact_prediction_raw(
    ...
    f_raw = phy["force"] + output[f"pred_force_delta_wheel{wheel_id}"]
    if not use_safety_gates:
        return f_raw
    ...
    gate = output.get(f"gate_force_wheel{wheel_id}", f_raw.new_ones(f_raw.shape))
    return lf_force + gate * (f_raw - lf_force)


def run_inference_v3(args: argparse.Namespace) -> None:
    ...
    scaler = restore_scaler_v3(args.checkpoint, ckpt)
    ...
    model_role = resolve_model_role_v3(args.model_role, ckpt)
    infer_teacher_future_len = teacher_future_len_ckpt if model_role == "teacher" else 0
    dataset = GraphTemporalSequenceDatasetV3(
        selected_df,
        spec,
        scaler,
    ...
    model, meta = build_model_from_checkpoint_v3(ckpt, spec, model_role, device)
    log_status("model loaded; starting batches")
    meta["history_len"] = history_len
    meta["teacher_future_len"] = infer_teacher_future_len
    use_safety_gates = bool(meta.get("uses_safety_gates", False))
    ...
    for batch in tqdm(loader, desc="Infer V3", dynamic_ncols=True):
        ...
        if use_safety_gates and len(body_pos_idx) == 3:
            pred_body_raw = apply_axis_gate(pred_body_raw, batch["lf_body_current"], body_pos_idx, output.get("gate_body_pos", pred_body_raw.new_ones(*pred_body_raw.shape[:2], 3)))
        ...
        for i in WHEEL_IDS:
            ...
            if use_safety_gates:
                ...
                omega_i = omega_index(wheel_cols)
                if len(wpos) == 3:
                    raw = apply_axis_gate(raw, batch[f"lf_wheel{i}_kin_current"], wpos, output.get(f"gate_wheel{i}_pos", raw.new_ones(*raw.shape[:2], 3)))
                if omega_i is not None:
                    raw = apply_axis_gate(raw, batch[f"lf_wheel{i}_kin_current"], [omega_i], output.get(f"gate_wheel{i}_omega", raw.new_ones(*raw.shape[:2], 1)))
        ...
        for i in WHEEL_IDS:
            ...
            pred_contact_raw = build_v3_contact_prediction_raw(batch, output, spec, scaler, pred_body_raw, pred_wheel_raw, i, sinkage_max, use_safety_gates)
    ...
    pred_path = os.path.join(args.output_dir, "predictions_v3.csv")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", type=str, required=True, help="V3 训练得到的 best_model.pt")
