...
from .data_utils_v4 import ROCKER_NAMES, WHEEL_IDS
...
class ForceTCNBlock(nn.Module):
    """LF-only wheel force direct path block, input/output [B, T, C]."""

    def __init__(self, channels: int, kernel_size: int, dilation: int, dropout: float, causal: bool):
        super().__init__()
        self.kernel_size = int(kernel_size)
        self.dilation = int(dilation)
        self.causal = bool(causal)
        self.conv = nn.Conv1d(channels, channels, self.kernel_size, dilation=self.dilation)
...
class ForceDirectTCN(nn.Module):
    """Shared wheel force direct TCN. Each wheel is passed through this same module."""
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            ForceTCNBlock(self.hidden_dim, kernel_size, int(d), dropout, causal)
            for d in (dilations or [1, 2, 4])
        ])

    def forward(self, x: torch.Tensor, current_index: int) -> torch.Tensor:
        if x.shape[-1] != self.input_dim:
            raise ValueError(f"force_direct_seq dim mismatch: got {x.shape[-1]}, expected {self.input_dim}")
        y = self.in_proj(x)
        for block in self.blocks:
            y = block(y)
        if current_index < 0 or current_index >= y.shape[1]:
            raise IndexError(f"current_index={current_index} out of range for force_direct_seq length {y.shape[1]}")
        return y[:, current_index:current_index + 1, :]


class ForceFusionHead(nn.Module):
...
class HGTLayer(nn.Module):
    """
    V4 本地 Heterogeneous Graph Transformer 层。
...
def build_hgt_vehicle_graph_v4(include_self_loops: bool = True) -> Tuple[Dict[str, int], List[str], torch.Tensor, torch.Tensor, List[str]]:
...
class HGTGraphTemporalCompensationModelV4(nn.Module):
    def __init__(
        self,
        ...
        dropout: float = 0.1,
        role: str = "student",
        use_force_tcn: bool = True,
        force_tcn_input_dim: int = 19,
        force_tcn_hidden: int = 64,
        force_tcn_kernel_size: int = 3,
        force_tcn_dilations: Optional[List[int]] = None,
        force_tcn_dropout: float = 0.1,
    ):
        super().__init__()
        ...
        self.role = role
        self.use_force_tcn = bool(use_force_tcn)
        self.force_tcn_input_dim = int(force_tcn_input_dim)
        self.force_tcn_hidden = int(force_tcn_hidden)
        self.force_tcn_kernel_size = int(force_tcn_kernel_size)
        self.force_tcn_dilations = list(force_tcn_dilations or [1, 2, 4])
        self.force_tcn_dropout = float(force_tcn_dropout)
        self._force_tcn_shape_logged = False
        ...
        node_map, node_types, edge_index, edge_type, relation_names = build_hgt_vehicle_graph_v4(True)
        ...
        self.force_fusion_dim = node_hidden_dim * 2 + (self.force_tcn_hidden if self.use_force_tcn else 0) + 6
        # Existing body residual channels remain delta_v and delta_p; the trainer
        # integrates corrected velocity and gates delta_p after propagation.
        # Rocker and wheel position channels remain body-frame local deltas, whose
        # gates must not scale the shared body-global translation.
        ...
        if self.use_force_tcn:
            self.force_direct_tcn = ForceDirectTCN(
                input_dim=self.force_tcn_input_dim,
                hidden_dim=self.force_tcn_hidden,
                kernel_size=self.force_tcn_kernel_size,
                dilations=self.force_tcn_dilations,
                dropout=self.force_tcn_dropout,
                causal=self.role == "student",
            )
            self.force_delta_head = ForceFusionHead(self.force_fusion_dim, dropout)
            self.force_gate_head = ResidualHead(self.force_fusion_dim, 3, head_hidden, dropout)
            print("[V4] Force direct TCN enabled", flush=True)
            print(f"[V4] Force direct input dim = {self.force_tcn_input_dim}", flush=True)
            print(f"[V4] Force TCN hidden = {self.force_tcn_hidden}", flush=True)
            print(f"[V4] Force TCN dilations = {self.force_tcn_dilations}", flush=True)
            print(f"[V4] Force fusion dim = {self.force_fusion_dim}", flush=True)
        else:
            self.force_direct_tcn = None
            ...
            print("[V4] Force direct TCN disabled; using original V4 force branch", flush=True)
    ...
    def forward(
        ...
        for i in WHEEL_IDS:
            ...
            if self.use_force_tcn:
                force_key = self._prefixed_key(f"force_direct_wheel{i}", input_prefix)
                if force_key not in batch:
                    raise KeyError(f"V4 ForceTCN requires batch['{force_key}']")
                fphy_key = f"wheel{i}_fphy_current"
                if fphy_key not in batch:
                    raise KeyError(f"V4 ForceTCN requires batch['{fphy_key}']")
                lf_force_key = f"wheel{i}_lf_force_current"
                if lf_force_key not in batch:
                    raise KeyError(f"V4 ForceTCN requires batch['{lf_force_key}']")
                force_direct_seq = batch[force_key]
                h_force = self.force_direct_tcn(force_direct_seq, target_idx)
                body_node = nodes[:, target_idx:target_idx + 1, self.body_idx, :]
                lf_force_current = batch[lf_force_key].to(device=force_direct_seq.device, dtype=force_direct_seq.dtype)
                fphy_current = batch[fphy_key].to(device=force_direct_seq.device, dtype=force_direct_seq.dtype)
                force_fusion = torch.cat([contact_node, body_node, h_force, lf_force_current, fphy_current], dim=-1)
                force_delta = self.force_delta_head(force_fusion)
                out[f"pred_force_delta_wheel{i}"] = force_delta
                out[f"gate_force_wheel{i}"] = torch.sigmoid(self.force_gate_head(force_fusion))
                z_force_parts.append(force_fusion)
                force_raw_parts.append(lf_force_current + out[f"gate_force_wheel{i}"] * force_delta)
                if not self._force_tcn_shape_logged:
                    print(f"[V4] force_direct_seq shape = {tuple(force_direct_seq.shape)}", flush=True)
                    print(f"[V4] h_force shape = {tuple(h_force.shape)}", flush=True)
                    print(f"[V4] h_wheel shape = {tuple(contact_node.shape)}", flush=True)
                    print(f"[V4] h_body shape = {tuple(body_node.shape)}", flush=True)
                    print(f"[V4] Fphy shape = {tuple(fphy_current.shape)}", flush=True)
                    print(f"[V4] force_fusion shape = {tuple(force_fusion.shape)}", flush=True)
                    print(f"[V4] force output shape = {tuple(force_delta.shape)}", flush=True)
                    self._force_tcn_shape_logged = True
            else:
                out[f"pred_force_delta_wheel{i}"] = self.force_delta_head(contact_feat)
                lf_force_key = f"wheel{i}_lf_force_current"
                if lf_force_key not in batch:
                    raise KeyError(f"V4 force branch requires batch['{lf_force_key}']")
                lf_force_current = batch[lf_force_key].to(device=contact_feat.device, dtype=contact_feat.dtype)
                out[f"gate_force_wheel{i}"] = torch.sigmoid(self.force_gate_head(contact_feat))
                z_force_parts.append(contact_feat)
                force_raw_parts.append(lf_force_current + out[f"gate_force_wheel{i}"] * out[f"pred_force_delta_wheel{i}"])
            ...
            if not self.use_force_tcn:
                out[f"gate_force_wheel{i}"] = torch.sigmoid(self.force_gate_head(contact_feat))
        out.update({k: v for k, v in stats.items() if v is not None})
        if len(z_force_parts) != len(WHEEL_IDS):
            raise RuntimeError("V4 force branch did not produce all six z_force features")
        out["z_force"] = torch.cat(z_force_parts, dim=1)
        out["force_raw"] = torch.cat(force_raw_parts, dim=1)
        return out


GraphTemporalHGTCompensationModelV4 = HGTGraphTemporalCompensationModelV4
